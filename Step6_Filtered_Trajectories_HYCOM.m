...
% recommandations: vérifier que le ConfigFile est dans le dossier de sortie
% sinon, le copier
% vérifier aussi les dossiers d'entrée et de sortie, moi c'est le même en
% fait

%% AEs
function Step6_Filtered_Trajectories_BoB(minlife, minamp, minrad, basepath)
%% récupération des vecteurs totaux L, A et R
close all

seuil_lifetime = minlife; %input('Minimum lifetime (days):   ');
seuil_Amp = minamp; %input('Minimum amplitude (cm):   ');
seuil_Radius = minrad; % input('Minimum radius (km):   ');
...
save(strcat(basepath,'EXTRACTION/ConfigFile_fulltime.mat'), 'Minimum_Radius_Trajectories', 'Minimum_Amplitude_Trajectories', 'Minimum_Duration_Trajectories');
addpath(strcat(basepath, 'FUNCTIONS'));
input_dir = [strcat(katepath,'/EDDY_TRAJECTORIES/')]; %EDDY_TRAJECTORIES/']; %EDDY_PROPERTIES'];
output_dir = [strcat(katepath,'/EDDY_TRAJECTORIES')]; %EDDY_TRAJECTORIES/'];
load([strcat(basepath,'EXTRACTION/EDDY_PROPERTIES/AE_Properties.mat')],'date_num','numtraj'); %EDDY_PROPERTIES/AE_Properties.mat'],'date_num','numtraj');
...
% Metadata().Filt_traj_Minimum_lifetime = seuil_lifetime;
% Metada().Filt_traj_Median_Amplitude = seuil_Amp;
% Metada().Filt_traj_Median_Radius = seuil_Radius;
...
%keep Metada input_dir output_dir seuil_lifetime seuil_Radius seuil_Amp

%% CEs

%% récupération des vecteurs totaux L, A et R

load([strcat(basepath,'EXTRACTION/EDDY_PROPERTIES/CE_Properties.mat')],'date_num','numtraj'); %CE_Properties.mat'],'date_num','numtraj');
...
% Metada().Filt_traj_Minimum_lifetime = seuil_lifetime;
% Metada().Filt_traj_Median_Amplitude = seuil_Amp;
% Metada().Filt_traj_Median_Radius = seuil_Radius;
