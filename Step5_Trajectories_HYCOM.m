...
%% load parameters
function Step5_Trajectories_BoB(basepath,cut,fullextract)
...
% n_traj = input('Number of trajectories per .mat when saved : ');   % 5000
% dist_lim = input('Maximum distance (in degree) of the closest centroid : '); % 3
% intersect_cover = input('Percentage of the minimum area intersection : '); % 75
% nDT = input('Maximum number of time steps allowed when searching for the next eddy : '); % 10
addpath(strcat(basepath, 'FUNCTIONS'));

%files_location = '/calcul/pc-pegliasco/Dyned/Extraction_eddies/EDDIES';
n_traj = 5000;
dist_lim = 2; %3
...
input_dir = [strcat(katepath, '/EDDY_PROPERTIES')]; %EDDY_PROPERTIES'];
...
% AEs=AEs(:,cut,:);
% Amplitude=Amplitude(:,cut);
% Area=Area(:,cut);
% date_num=date_num(cut);
% Nanti=Nanti(:,cut);
% Radius=Radius(:,cut);
% Xcenter=Xcenter(:,cut);
% Xcentroid=Xcentroid(:,cut);
% Ycenter=Ycenter(:,cut);
% Ycentroid=Ycentroid(:,cut);
            ...
            for DT=1:nDT % possibility of searching until nDT timesteps
                ...
                if isempty(next_map_eddies)==0 % if a close eddy is found
                    ...
                    if isempty(matching_contour)==0 && length(matching_contour)==1 % only one contour
                        ...
                        %  disp(['eddy  ' num2str(eddy_next)  '  ' num2str(indy+DT)])
                    ...
                    elseif isempty(matching_contour)==0 && length(matching_contour) >1 % more than 1 contour -> cost function
                        ...
                        if length(sev_areas)>1 % splitting event -> reccord the eddies generated
                            ...
                            %      disp(['Splitting ' num2str(matching_contour(sev_areas(1))) ' ' num2str(matching_contour(sev_areas(2)))  ' ' num2str(indy+DT)])
