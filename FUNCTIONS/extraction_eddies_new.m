%%
function [Anticyclonic_Cell,Cyclonic_Cell,Nanti,Ncyclo] = extraction_eddies_new(X,Y,SSH,minDist,min_amp,min_R,Extraction_Type)
...
% f=sort([f1(:);f2(:)]);
% n= round(0.75/(diff(X(1:2))));
% f1 = find(MatrixLocalMax(SSH,n)==1);
% f2 = find(MatrixLocalMax(-SSH,n)==1);
% f=sort([f1(:);f2(:)]);
...
for i=1:length(f)
    ...
    if isempty(iind)==0 && iind(1)>=min_amp & length(isfinite(ssh(iiind)))>=4 & length(unique(xxx(iiind)))>=2 & length(unique(yyy(iiind)))>=2 
